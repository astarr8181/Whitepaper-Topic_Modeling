...
def main():
    ...
    # Run the script in the directory containing the files to be parsed or reset the path to the file location
    folder = './'
    ...
    for document in word_docs:
        text += [read_word_documents(document)]
        
    pdf_text = []
    for pdf in pdfs:
        try:
            pdf_text += [read_pdfs(pdf)]
        except:
    ...
    output.to_csv('white_papers_text.csv')


# function parses text from docx files by paragraph and aggregates it
def read_word_documents(file_name):
